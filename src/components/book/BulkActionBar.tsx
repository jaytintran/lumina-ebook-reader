...
export function BulkActionBar() {
  ...
  const removeFromFolder = useRemoveBooksFromFolder();
  const { data: collections = [] } = useCollections();
  const [editOpen, setEditOpen] = useState(false);

  const folderParam = searchParams.get("folder");
  const currentFolderId =
    folderParam && !Number.isNaN(Number(folderParam)) ? Number(folderParam) : null;
  ...
  const handleRemoveFromFolder = (folderId: number) => {
    removeFromFolder.mutate({ bookIds: ids, folderId });
    clearSelection();
  };
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        {currentFolderId && (
          <Button
            size="sm"
            variant="outline"
            className="h-8 gap-1.5 rounded-lg text-xs font-medium shadow-2xs hover:bg-destructive/10 hover:text-destructive hover:border-destructive/30"
            onClick={() => handleRemoveFromFolder(currentFolderId)}
          >
            ...
            <DropdownMenuContent align="end" className="min-w-48 p-1">
              ...
              <DropdownMenuGroup>
                ...
                {currentFolderId ? (
                  <DropdownMenuItem
                    onClick={() => handleRemoveFromFolder(currentFolderId)}
                    className="cursor-pointer gap-2 rounded-md px-2.5 py-1.5 text-xs text-rose-500 hover:text-rose-600 focus:text-rose-600"
                  >
                    <FolderMinus className="h-3.5 w-3.5" />
                    <span className="truncate">Remove from this Folder</span>
                  </DropdownMenuItem>
                ) : (
                  folders.map((f) => (
                    <DropdownMenuItem
